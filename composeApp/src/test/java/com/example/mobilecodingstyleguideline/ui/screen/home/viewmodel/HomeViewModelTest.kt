...
class HomeViewModelTest {
    ...
    fun setUp() {
        ...
        viewmodel = HomeViewModel(
            ...
            editStatusSupplierUseCase = editStatusSupplierUseCase
        )
    ...
    fun `when onRefresh, search query must be empty, filter data must be empty, and show search must be false`() {
        runTest {
            // Arrange
            val supplierList = listOf(SupplierEntity())
            arrangeGetSupplierUseCase(
                params = GetSupplierQueryParams(),
                result = Result.Success(supplierList)
            )
            ...
            assertThat(viewmodel.uiState.value.supplier).isEqualTo(supplierList)
        }
    }

    @Test
    fun `when onRefresh is called, update uiState and populate supplier`() {
        runTest {
            // Arrange
            val supplierList = listOf(SupplierEntity())
            ...
            } returns flowOf(Result.Success(supplierList))

            coEvery {
                getSupplierOptionUseCase(GetSupplierOptionQueryParams())
            } returns flowOf(Result.Success(SupplierOptionEntity()))
    ...
    @Test
    fun `when onRefresh is called but returns error, supplier must be empty`() {
        runTest {
            // Arrange
            val errorMsg = "Unknown Error"
            coEvery {
                getSuppliersUseCase(GetSupplierQueryParams())
            } returns flowOf(Result.Error(errorMsg))
